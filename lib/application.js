...
import mitt from './mitt/dist/mitt';
import Context from './context';
import DOM from './dom-native';
import DOMEventDelegate from './dom-event-delegate';
	...
	*/
		...
		for (i = 0; i < behaviorNames.length; i++) {
			...
			if (behaviorName in includedBehaviors) {
				...
				if (!moduleBehaviorInstances[behaviorName]) {
					let instance = behaviorData.creator(instanceData.context);
					moduleBehaviorInstances[behaviorName] = instance;
					
					if(instance.messages && instance.onmessage) {
						instance.messages.forEach(e=>{
							let handler;
							if(typeof instance.onmessage === 'object') {
	...
	return assign(application, {
		...
		start: function(element) {
			...
			if (!this.isStarted(element)) {
				...
				if (jsmodule.messages && jsmodule.onmessage) {
					...
					jsmodule.messages.forEach(e=>{
						let handler;
						if(typeof jsmodule.onmessage === 'object') {
		...
		startAll: function(root) {
			var moduleElements = DOM.queryAll(root, MODULE_SELECTOR);
		...
		getModuleConfig: function(element, name) {
			...
			if (instanceData && instanceData.config) {
				...
				var configElement = DOM.query(element, 'script[type="text/x-config"]');
